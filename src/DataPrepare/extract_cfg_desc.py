...
def extract_vul_pat((funname, filename), log_file, output_dir):
    line_num_str = safe_exec_ret("cat %s | grep -n \'%s(.*)\' -A1 | grep '(ENTRY)\]' | grep -o '[0-9]*' | head -n 1" % (log_file, funname) )
    ...
    for index in range(len(entry_line_items)):
        if entry_line_items[index].startswith(line_num_str):
            break
    if not entry_line_items[index].startswith(line_num_str):
        exit(0)
    fd = open(log_file, 'r')
    print 'index:', index
    ...
    new_func_file = "%s#%s" % (filename, funname)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 4:
        print "extract_cfg_desc.py <affected_only_or_not> <graph_log> <output_folder>\n"
        exit(-1)
    ...
    if Match:
        ...
        traverse_log(sys.argv[2], sys.argv[3])
